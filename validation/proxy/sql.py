...
async def get_api_key_info(
    ...
    async with db_lock:
        async with conn.execute(
            f"SELECT {KEY}, {BALANCE}, {RATE_LIMIT_PER_MINUTE} FROM {API_KEYS_TABLE} WHERE {KEY} = ?",
            (api_key,),
        ) as cursor:
...
async def get_all_api_keys(conn: aiosqlite.Connection) -> List[Dict[str, Any]]:
    async with db_lock:
        async with conn.execute(
            f"SELECT {KEY}, {BALANCE}, {RATE_LIMIT_PER_MINUTE} FROM {API_KEYS_TABLE}"
        ) as cursor:
            return [
                {
                    KEY: row[0],
...
async def get_all_logs_for_key(
    conn: aiosqlite.Connection, api_key: str
) -> List[Dict[str, Any]]:
    async with db_lock:
        async with conn.execute(
...
async def update_api_key_name(conn: aiosqlite.Connection, key: str, name: str):
    async with db_lock:
        await conn.execute(
            f"UPDATE {API_KEYS_TABLE} SET {NAME} = ? WHERE {KEY} = ?", (name, key)
        )
        await conn.commit()
...
async def update_requests_and_credits(
    conn: aiosqlite.Connection, api_key_info: Dict[str, Any], cost: float
) -> None:
    async with db_lock:
        await conn.execute(
...
async def log_request(
    conn: aiosqlite.Connection, api_key_info: Dict[str, Any], path: str, cost: float
) -> None:
    async with db_lock:
...
async def rate_limit_exceeded(
    conn: aiosqlite.Connection, api_key_info: Dict[str, Any]
) -> bool:
    async with db_lock:
