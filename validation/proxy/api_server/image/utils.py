...
def do_formatted_response_image_checks(
    formatted_response: BaseModel, result: utility_models.QueryResult
):
    ...
    if formatted_response is None or formatted_response.image_b64 is None:
        ...
        bt.logging.error(
            f"Received a None result for some reason; Result error message: {result.error_message}"
        )
